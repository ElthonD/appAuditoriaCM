...
def createPage():
    ...
    try:
        ...
        st.markdown("<h2 style='text-align: left;'>Datos de las auditorías para Centro de Monitoreo y Línea de Reacción</h2>", unsafe_allow_html=True)
        st.write(f'Este módulo contiene los datos de las auditoría realizados a Centro de Monitoreo y Línea de Reacción {df.Mes.values[0]} {df.Año.values[0].astype(int)} a {df.Mes.values[-1]} {df.Año.values[-1].astype(int)} .')
        ...
        # Mostrar informes
        st.markdown("<h2 style='text-align: left;'>Informes de Auditorías para Centro de Monitoreo y Línea de Reacción</h2>", unsafe_allow_html=True)
        st.write(f'Este módulo contiene informes de auditoría realizados a Centro de Monitoreo y Línea de Reacción {df.Mes.values[0]} {df.Año.values[0].astype(int)} a {df.Mes.values[-1]} {df.Año.values[-1].astype(int)} .')
        
        uploaded_file = st.file_uploader('Seleccionar Informe de Auditoría (.pdf)', type="pdf", help="Solo archivos PDF son soportados", )
